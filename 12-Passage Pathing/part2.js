...
const isSmallVisitedTwicwe = (path) => {
    const smallCaves = path.filter(cave => cave.isSmall && !cave.isStart && !cave.isEnd).map(cave => cave.name);
    const uniqueSmallCavesCount = new Set(smallCaves).size;
    return smallCaves.length !== uniqueSmallCavesCount;
}
